...
if [ "$1" == "--logout" ]; then
	curl --insecure -A "$USER_AGENT" "$LOGOUT_URL" || echo "${RED_BEGIN}Failed to log you out${NORMAL}"
else
	...
	if curl --insecure -A "$USER_AGENT" "$CHECK_STATUS_URL" 2>/dev/null | grep -q "re now logged on to BT Wi-Fi"; then
		echo "${GREEN_BEGIN}you're logged in${NORMAL}"
	else
		echo "Trying to log you in..."
		if wget --no-check-certificate -qO - --header='Accept: text/html' --user-agent "$USER_AGENT" --post-data "inputUsername=${USERNAME}&username=wam/${USERNAME}&password=${PASSWORD}" $LOGIN_URL 2>/dev/null | grep -q "re now logged on to BT Wi-Fi"; then
